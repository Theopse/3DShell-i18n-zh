...
void Dirbrowse_DisplayFiles(void) {
	...
	Draw_GetTextSize(0.42f, NULL, &title_height, cwd);
	Draw_Textf(70, 18 + ((34 - title_height) / 2) - 3, 0.42f, WHITE, strlen(cwd) > 45? "%.45s..." : "%s", cwd);
		...
		if (position < FILES_PER_PAGE || i > (position - FILES_PER_PAGE)) {
			...
			if (!strncmp(file->name, "..", 2))
				...
				if (!file->isDir) {
					...
					Draw_Text((398 - file_size_width), 52 + (38 - file_size_height - 5) + (38 * printed) - 2, 0.4f, config.dark_theme? WHITE : BLACK, file_size);
				}
			}
